...
Payload LightingAndShadows::Hit(const Ray& ray, const IntersectableData& data, const MaterialTriangle* triangle, const unsigned int max_raytrace_depth) const
{
    ...
    payload.color = triangle->emissive_color +triangle->ambient_color * 0.1f;
    ...
    for (auto& light : lights) {
        ...
        if (fabs(t - to_light_length) < 0.001f) {
            payload.color += light->color * triangle->diffuse_color * std::max(0.f, dot(to_light_ray.direction, normal)); //Diffuse

            Ray from_light_ray(light->position, x - light->position);
            float3 specular_direction = from_light_ray.direction - 2.f * dot(from_light_ray.direction, normal) * normal; // Specular
